...
if (mysqli_connect_errno ()) { // verifica si ha habido un error
	...
	$reference=$_GET['id'];
	$consulta = "SELECT partido_visitante FROM partido where partido_id=?";
	
	if ($stmt = $conn->prepare ($consulta)) {
		$stmt->bind_param ('s',$reference);  //define los parametros que recibe la funcion
		$stmt->execute ();
		/* bind variables to prepared statement */
		$stmt->bind_result ($team_visita);
		$stmt->fetch ();
		/* close statement */
		$stmt->close ();
		};
	...
	if ($stmt = $conn->prepare ($consulta)) {
		$stmt->bind_param ( 'i',$team_visita);  //define los parametros que recibe la funcion
		$stmt->execute ();
		...
		while ( $stmt->fetch () ) {
			...
			echo '<option value='.$jugador_id.'>'.$jugador_nombre.'</option>';// colocacion de los datos consultados en el droplist
        }
