...
$conn = new mysqli ("127.0.0.1:3306", "base1", "base", "stm"); //crea la conexion con la base de datos
 ...
 $conn1 = new mysqli ("127.0.0.1:3306", "base1", "base", "stm"); //crea la conexion con la base de datos
 if (mysqli_connect_errno ()) { //verifica si ha habido un error
   $error = mysqli_connect_error (); //guarda el mensaje de error para mostrarlo en la pagina
 } else {
      ...
      if ($stmt1 = $conn1->prepare ( $query )) { //verifica que la sentencia haya sido preparada para su ejecucion
              $stmt1->bind_param ( 's', $user);  //define los parametros que recibe la funcion
              $stmt1->execute ();                             //ejecuta el query
              $stmt1->bind_result ( $passBase );				//define a la variable $resultado, donde se va a almacenar el resultado
              $stmt1->fetch ();								//guarda el resultado en la variable $resultado
              $stmt1->close (); //se cierra el query
      }
 }
?>
