...
//$error=''; // Variable To Store Error Message
    ...
    if (mysqli_connect_errno ()) { //verifica si ha habido un error
        ...
        $consulta = "select select_contraseña(?)";    //define la funcion loguear, que se va a ejecutar en la base de datos para verificar si nickname y password son correctos
    ...
    if (empty ( $_POST ['campo_contrasenaNueva'])) {//revisa si contraseña nueva esta vacio
        ...
        if ($contrasenaAnterior!=$contrasenaEnBase ){
            ...
            $scriptUpdateUsuarioContraseña='call update_usuarioContraseña(?,?)'; //guarda en una variable local la instruccion a ejecutar
            if ($stmt = $conn->prepare ( $scriptUpdateUsuarioContraseña )) { //verifica que la sentencia haya sido preparada para su ejecucion
    ...
    if (empty ( $_POST ['campo_correo'] )) {//se verifica que ningun campo este vacio
            ...
            if ($contrasenaBeforeEncry!=$contrasenaEnBase ){
                ...
                $scriptUpdateUsuarioCorreo='call update_administradorCorreo(?,?)'; //guarda en una variable local la instruccion a ejecutar
                if ($stmt = $conn->prepare ( $scriptUpdateUsuarioCorreo )) { //verifica que la sentencia haya sido preparada para su ejecucion
